...
export class Game {
  ...
  private redrawNextCanvas(): void {
    ...
    if (next) {
      const xOffset = next.dimension === 2 ? 20 : next.dimension === 3 ? 10 : next.dimension === 4 ? 0 : null;
      const yOffset = next.dimension === 2 ? 20 : next.dimension === 3 ? 20 : next.dimension === 4 ? 10 : null;
  ...
  private startWorkingPieceDropAnimation(callback: () => void = () => { }): void {
  ...
  private toggleAI(): void {
    ...
    this.aiButton.style.backgroundColor = this.isAIActive ? '#E9E9FF' : '#FF3232';
  }
